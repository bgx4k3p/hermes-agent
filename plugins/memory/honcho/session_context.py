...
_FAILED = (
    object()
)  # sentinel: a guarded call raised (distinct from a legitimately empty/None result)
...
class SessionContextMixin:
    ...
    def _guarded(
        self, fn: Callable[[], Any], default: Any, level: int, msg: str, *args: Any
    ) -> Any:
    ...
    def _guarded_authed(
        ...
        *args: Any,
        ...
        return self._guarded(
            lambda: self._authed_call(label, fn), default, level, msg, *args
        )
    ...
    def _guarded_session(
        ...
        *args: Any,
        ...
        return (
            self._guarded(lambda: fn(session), default, level, msg, *args)
            if session
            else default
        )
    ...
    def _fetch_peer_card(self, peer_id: str, *, target: str | None = None) -> list[str]:
        ...
        def _get_card() -> Any:
            ...
            getters = (
                getattr(peer, n, None) for n in ("get_card", "card")
            )  # "card" is the legacy SDK getter
    ...
    def _fetch_peer_context(
        ...
        *,
        ...
        ctx = self._guarded_authed(
            "peer context fetch",
            lambda: peer().context(**context_kwargs),
            None,
            logging.DEBUG,
            failed % "peer.context()",
            peer_id,
        ...
        if not representation:
            representation = (
                self._guarded_authed(
                    "peer representation fetch",
                    lambda: peer().representation(**self._target_kwargs(target)),
                    "",
                    logging.DEBUG,
                    failed % "peer.representation()",
                    peer_id,
                )
                or ""
            )
        if not card:
            card = self._guarded(
                ...
                [],
                logging.DEBUG,
                failed % "peer card fetch",
                peer_id,
            )
        return {"representation": representation, "card": card}

    def _peer_context_strings(
        self,
        peer_id: str,
        search_query: str | None = None,
        *,
    ...
    def get_prefetch_context(
        ...
        for step, level, msg in (
            (
                _summary,
                logging.DEBUG,
                "Failed to fetch session summary from Honcho: %s",
            ),
    ...
    def get_session_context(
        ...
        return self._guarded(
            _fetch, {}, logging.DEBUG, "Session context fetch failed: %s"
        )
    ...
    def get_peer_card(self, session_key: str, peer: str = "user") -> list[str]:
        ...
        def _fetch(session: Any) -> list[str]:
            ...
            return card or (
                self._fetch_peer_card(target_peer_id) if target_peer_id else []
            )
        ...
        return self._guarded_session(
            ...
            [],
            logging.DEBUG,
            "Failed to fetch peer card from Honcho: %s",
        )
    ...
    def search_context(
        ...
        q = (query or "").strip()[
            :4000
        ]  # Honcho caps query length for the embedding model.
        ...
        messages = self._guarded_authed(
            ...
            lambda: self.honcho.search(
                q, filters={"peer_perspective": peer_id}, limit=limit
            ),
            _FAILED,
            logging.DEBUG,
            "Honcho message search failed (peer_perspective=%s): %s",
            peer_id,
        )
        if messages is _FAILED or not messages:
            # Older Honcho versions may reject the perspective filter, while v3.0.12
            # can accept it but return an empty compatibility response. Both cases
            # fall back to the supported peer-authored search path.
            messages = self._guarded_authed(
                "peer search",
                lambda: self._get_or_create_peer(peer_id).search(q, limit=limit),
                None,
                logging.DEBUG,
                "Honcho peer search fallback also failed: %s",
            )
        if not messages:
            return ""
    ...
    def _conclusions_scope(self, session: Any, target_peer_id: str) -> Any:
        ...
        ai_observes = (
            target_peer_id == session.assistant_peer_id or self._ai_observe_others
        )
        observer = self._get_or_create_peer(
            session.assistant_peer_id if ai_observes else target_peer_id
        )
    ...
    def create_conclusion(
        ...
        return self._guarded(
            _create, False, logging.ERROR, "Failed to create conclusion: %s"
        )
    ...
    def delete_conclusion(
        ...
        return self._guarded_session(
            ...
            False,
            logging.ERROR,
            "Failed to delete conclusion %s: %s",
            conclusion_id,
        )
    ...
    def list_conclusions(
        ...
        def _list(session: Any) -> list[dict]:
            ...
            def _fetch() -> Any:
                ...
                return (
                    scope.query(query, top_k=limit)
                    if query
                    else scope.list(size=limit).items
                )
        ...
        return self._guarded_session(
            session_key, _list, [], logging.DEBUG, "Honcho list_conclusions failed: %s"
        )
    ...
    def set_peer_card(
        ...
        return self._guarded_session(
            session_key, _update, None, logging.ERROR, "Failed to set peer card: %s"
        )
    ...
    def get_ai_representation(self, session_key: str) -> dict[str, str]:
        ...
        def _fetch(session: Any) -> dict[str, str]:
            rep, card = self._peer_context_strings(
    ...
    def dialectic_query(
        ...
        level = (
            ...
            else self._dialectic_reasoning_level
        )
        ...
        def _chat_once() -> str:
            ...
            if self._ai_observe_others and target_peer_id != session.assistant_peer_id:
                ...
                return (
                    observer.chat(query, target=target_peer_id, reasoning_level=level)
                    or ""
                )
            return (
                self._get_or_create_peer(target_peer_id).chat(
                    query, reasoning_level=level
                )
                or ""
            )
        ...
        try:
            ...
            return (
                result[:cap].rsplit(" ", 1)[0] + " …"
                if result and cap and len(result) > cap
                else result
            )
        except HonchoAuthError:
            raise
